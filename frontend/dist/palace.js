import store, { updateItem } from "./store.js";
...
const moveCamera = (direction) => {
    ...
    if (direction === ANIMATION_ID.camera_left_to_right &&
        MAPS[MAPS.length - 1].offsetLeft <= 0) {
        ANIMATION_RUNNING_VALUES[ANIMATION_ID.camera_left_to_right] = 0;
        return;
    }
    MAPS.forEach((map) => (map.style.left = `${map.offsetLeft +
        (direction === ANIMATION_ID.camera_left_to_right ? -1 : 1) * 4}px`));
...
const launchCharacterAnimation = (characterElement, throttleNum, extension, spriteBase, spriteIndex, max, min, loop, animationId) => {
    ...
    if (!ANIMATION_RUNNING_VALUES[animationId] ||
...
const checkForScreenUpdateFromLeftToRight = (throttleNum) => {
    ...
    if (lastMapDomElement &&
        lastMapDomElement.offsetLeft <= window.innerWidth / 10 &&
        currentCacheRightIndex < window.store.getState().localStorage.length - 1) {
        MAPS.push(createMapPalaceBlock(lastMapDomElement.offsetLeft + lastMapDomElement.offsetWidth, window.store.getState().localStorage[currentCacheRightIndex]));
...
window.onload = () => {
    ...
    MAPS.push(createMapPalaceBlock(window.innerWidth, window.store.getState().localStorage[1]));
};
let searchTimeout;
...
const changeSlotItem = (src) => {
    ...
    window.store.dispatch(updateItem({ blockIndex, itemIndex, item: { id: makeId(3), src } }));
};
